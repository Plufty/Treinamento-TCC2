...
def BcelAdam (learningRate, data_dir):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        print('Epoch [%d], epoch time: %.3f s, train loss: %.3f, val loss: %.3f, train accuracy: %.3f %%, val accuracy: %.3f %%' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
        perdas_efficient.append('EfficientNet,Binary_Cross_Entropy_Loss,Adam,%d,%.3f,%.3f,%.3f,%.3f,%.3f\n' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        print('Epoch [%d], epoch time: %.3f s, train loss: %.3f, val loss: %.3f, train accuracy: %.3f %%, val accuracy: %.3f %%' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
        perdas_res.append('ResNet50,Binary_Cross_Entropy_Loss,Adam,%d,%.3f,%.3f,%.3f,%.3f,%.3f\n' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        print('Epoch [%d], epoch time: %.3f s, train loss: %.3f, val loss: %.3f, train accuracy: %.3f %%, val accuracy: %.3f %%' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
        perdas_shuffle.append('ShuffleNet,Binary_Cross_Entropy_Loss,Adam,%d,%.3f,%.3f,%.3f,%.3f,%.3f\n' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        print('Epoch [%d], epoch time: %.3f s, train loss: %.3f, val loss: %.3f, train accuracy: %.3f %%, val accuracy: %.3f %%' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
        perdas_squeeze.append('SqueezeNet,Binary_Cross_Entropy_Loss,Adam,%d,%.3f,%.3f,%.3f,%.3f,%.3f\n' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        print('Epoch [%d], epoch time: %.3f s, train loss: %.3f, val loss: %.3f, train accuracy: %.3f %%, val accuracy: %.3f %%' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
        perdas_vgg.append('VGG16,Binary_Cross_Entropy_Loss,Adam,%d,%.3f,%.3f,%.3f,%.3f,%.3f\n' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
    ...
    torch.save(model_alex.state_dict(), data_dir+'\modelo_treinado_BCEL_Adam_BCEL_alex_learningRate{}.pth'.format(learningRate))
    print("Modelo_alex treinado salvo com sucesso!")
    arquivo = open(data_dir+'\perdas_alex_Bcel_Adam_learningRate{}.csv'.format(learningRate),'w')
    for epoch in tqdm(range(num_epochs+2)):
        arquivo.write(perdas_alex[epoch])
    arquivo.close()

    torch.save(model_efficient.state_dict(), data_dir+'\modelo_treinado_BCEL_Adam_BCEL_efficient_learningRate{}.pth'.format(learningRate))
    print("Modelo_efficient treinado salvo com sucesso!")
    arquivo = open(data_dir+'\perdas_efficient_Bcel_Adam_learningRate{}.csv'.format(learningRate),'w')
    for epoch in tqdm(range(num_epochs+2)):
        arquivo.write(perdas_efficient[epoch])
    arquivo.close()

    torch.save(model_res.state_dict(), data_dir+'\modelo_treinado_BCEL_Adam_BCEL_res_learningRate{}.pth'.format(learningRate))
    print("Modelo_res treinado salvo com sucesso!")
    arquivo = open(data_dir+'\perdas_res_Bcel_Adam_learningRate{}.csv'.format(learningRate),'w')
    for epoch in tqdm(range(num_epochs+2)):
        arquivo.write(perdas_res[epoch])
    arquivo.close()


    torch.save(model_shuffle.state_dict(), data_dir+'\modelo_treinado_BCEL_Adam_BCEL_shuffle_learningRate{}.pth'.format(learningRate))
    print("Modelo_shuffle treinado salvo com sucesso!")
    arquivo = open(data_dir+'\perdas_shuffle_Bcel_Adam_learningRate{}.csv'.format(learningRate),'w')
    for epoch in tqdm(range(num_epochs+2)):
        arquivo.write(perdas_shuffle[epoch])
    arquivo.close()


    torch.save(model_squeeze.state_dict(), data_dir+'\modelo_treinado_BCEL_Adam_BCEL_squeeze_learningRate{}.pth'.format(learningRate))
    print("Modelo_squeeze treinado salvo com sucesso!")
    arquivo = open(data_dir+'\perdas_squeeze_Bcel_Adam_learningRate{}.csv'.format(learningRate),'w')
    for epoch in tqdm(range(num_epochs+2)):
        arquivo.write(perdas_squeeze[epoch])
    arquivo.close()


    torch.save(model_vgg.state_dict(), data_dir+'\modelo_treinado_BCEL_Adam_BCEL_vgg_learningRate{}.pth'.format(learningRate))
    print("Modelo_vgg treinado salvo com sucesso!")
    arquivo = open(data_dir+'\perdas_vgg_Bcel_Adam_learningRate{}.csv'.format(learningRate),'w')
    for epoch in tqdm(range(num_epochs+2)):
