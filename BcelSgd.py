...
def BcelSGD (learningRate, data_dir):
    ...
    optimizer = optim.SGD(model_alex.parameters(), lr=learningRate, momentum=0.9)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for i, (inputs, labels) in enumerate(dataloaders['train'], 0):
            ...
            predicted_train = predicted_train.view(-1, 1)
        ...
        with torch.no_grad():
            for j, (inputs_val, labels_val) in enumerate(dataloaders['val'], 0):
                ...
                labels_val = labels_val.unsqueeze(1).float()
                labels_val = labels_val.repeat(1, 2)
    ...
    # # EfficientNet
    optimizer = optim.SGD(model_efficient.parameters(), lr=learningRate, momentum=0.9)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        with torch.no_grad():
            for j, (inputs_val, labels_val) in enumerate(dataloaders['val'], 0):
                ...
                # reshape inputs_val e labels_val, se necessário
        ...
        print('Epoch [%d], epoch time: %.3f s, train loss: %.3f, val loss: %.3f, train accuracy: %.3f %%, val accuracy: %.3f %%' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
        perdas_efficient.append('EfficientNet,Binary_Cross_Entropy_Loss,SGD,%d,%.3f,%.3f,%.3f,%.3f,%.3f\n' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
    ...
    # # ResNet50
    optimizer = optim.SGD(model_res.parameters(), lr=learningRate, momentum=0.9)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        print('Epoch [%d], epoch time: %.3f s, train loss: %.3f, val loss: %.3f, train accuracy: %.3f %%, val accuracy: %.3f %%' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
        perdas_res.append('ResNet50,Binary_Cross_Entropy_Loss,SGD,%d,%.3f,%.3f,%.3f,%.3f,%.3f\n' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
    ...
    # # ShuffleNet
    optimizer = optim.SGD(model_shuffle.parameters(), lr=learningRate, momentum=0.9)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        print('Epoch [%d], epoch time: %.3f s, train loss: %.3f, val loss: %.3f, train accuracy: %.3f %%, val accuracy: %.3f %%' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
        perdas_shuffle.append('ShuffleNet,Binary_Cross_Entropy_Loss,SGD,%d,%.3f,%.3f,%.3f,%.3f,%.3f\n' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
    ...
    # SqueezeNet
    optimizer = optim.SGD(model_squeeze.parameters(), lr=learningRate, momentum=0.9)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        print('Epoch [%d], epoch time: %.3f s, train loss: %.3f, val loss: %.3f, train accuracy: %.3f %%, val accuracy: %.3f %%' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
        perdas_squeeze.append('SqueezeNet,Binary_Cross_Entropy_Loss,SGD,%d,%.3f,%.3f,%.3f,%.3f,%.3f\n' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
    ...
    # VGG16
    optimizer = optim.SGD(model_vgg.parameters(), lr=learningRate, momentum=0.9)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        print('Epoch [%d], epoch time: %.3f s, train loss: %.3f, val loss: %.3f, train accuracy: %.3f %%, val accuracy: %.3f %%' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
        perdas_vgg.append('VGG16,Binary_Cross_Entropy_Loss,SGD,%d,%.3f,%.3f,%.3f,%.3f,%.3f\n' % (epoch+1, epoch_fim-epoch_ini, train_loss, val_loss, train_accuracy, val_accuracy))
    ...
    torch.save(model_alex.state_dict(), data_dir+'\modelo_treinado_BCEL_SGD_alex_learningRate{}.pth'.format(learningRate))
    print("Modelo_alex treinado salvo com sucesso!")
    arquivo = open(data_dir+'\perdas_alex_Bcel_SGD_learningRate{}.csv'.format(learningRate),'w')
    for epoch in tqdm(range(num_epochs+2)):
        arquivo.write(perdas_alex[epoch])
    arquivo.close()

    torch.save(model_efficient.state_dict(), data_dir+'\modelo_treinado_BCEL_SGD_efficient_learningRate{}.pth'.format(learningRate))
    print("Modelo_efficient treinado salvo com sucesso!")
    arquivo = open(data_dir+'\perdas_efficient_Bcel_SGD_learningRate{}.csv'.format(learningRate),'w')
    for epoch in tqdm(range(num_epochs+2)):
        arquivo.write(perdas_efficient[epoch])
    arquivo.close()

    torch.save(model_res.state_dict(), data_dir+'\modelo_treinado_BCEL_SGD_res_learningRate{}.pth'.format(learningRate))
    print("Modelo_res treinado salvo com sucesso!")
    arquivo = open(data_dir+'\perdas_res_Bcel_SGD_learningRate{}.csv'.format(learningRate),'w')
    for epoch in tqdm(range(num_epochs+2)):
        arquivo.write(perdas_res[epoch])
    arquivo.close()


    torch.save(model_shuffle.state_dict(), data_dir+'\modelo_treinado_BCEL_SGD_shuffle_learningRate{}.pth'.format(learningRate))
    print("Modelo_shuffle treinado salvo com sucesso!")
    arquivo = open(data_dir+'\perdas_shuffle_Bcel_SGD_learningRate{}.csv'.format(learningRate),'w')
    for epoch in tqdm(range(num_epochs+2)):
        arquivo.write(perdas_shuffle[epoch])
    arquivo.close()


    torch.save(model_squeeze.state_dict(), data_dir+'\modelo_treinado_BCEL_SGD_squeeze_learningRate{}.pth'.format(learningRate))
    print("Modelo_squeeze treinado salvo com sucesso!")
    arquivo = open(data_dir+'\perdas_squeeze_Bcel_SGD_learningRate{}.csv'.format(learningRate),'w')
    for epoch in tqdm(range(num_epochs+2)):
        arquivo.write(perdas_squeeze[epoch])
    arquivo.close()


    torch.save(model_vgg.state_dict(), data_dir+'\modelo_treinado_BCEL_SGD_vgg_learningRate{}.pth'.format(learningRate))
    print("Modelo_vgg treinado salvo com sucesso!")
    arquivo = open(data_dir+'\perdas_vgg_Bcel_SGD_learningRate{}.csv'.format(learningRate),'w')
    for epoch in tqdm(range(num_epochs+2)):
